                            ...
                            <div className="text-center overflow-hidden bg-white" key={activity._id}>
                                ...
                                {/* when a settlement has been done and waiting in process */}
                                {
                                    activity.type==='PAYMENT_SENT' && (
                                        <div className=" rounded-xl p-4 border-l-5 shadow-md flex justify-center gap-2 flex-wrap border-l-yellow-500"> 
                                            ...
                                            💸 <b>{activity.performedBy[0]===userDetails._id?"You":userMap[activity.performedBy[0]]}</b> paid <b className="text-yellow-500">₹{activity.amount}</b> to <b>{activity.performedFor[0]===userDetails._id?"You":userMap[activity.performedFor[0]]}</b> 
                                            {"  "}
                                            <span className="bg-yellow-100 text-orange-500 text-xs p-1 px-4 border border-yellow-200">
                                                ⏳ Waiting for {activity.performedFor[0]===userDetails._id?"You":userMap[activity.performedFor[0]]} to confirm...
                                            </span>
                                        </div>
                                    )
                                }
                                ...
                                {/* settlement has been accepted by the reciver */}
                                {
                                    activity.type==='PAYMENT_CONFIRMED' && (
                                        <div className=" rounded-xl p-4 border-l-5 shadow-md border-green-500">✅ <b>{activity.performedBy[0]===userDetails._id?"You":userMap[activity.performedBy[0]]}</b> confirmed receiving <b className="text-green-500">₹{activity.amount}</b>  from <b>{activity.performedFor[0]===userDetails._id?"You":userMap[activity.performedFor[0]]}</b></div>
                                    )
                                }
                                ...
                                {/* payment is declined by reciever */}
                                {
                                    activity.type==='PAYMENT_DECLINED' && (
                                        <div className="rounded-xl p-4 border-l-5 shadow-md border-red-600 ">❌ <b>{activity.performedBy[0]===userDetails._id?"You":userMap[activity.performedBy[0]]}</b> declined <b className="text-red-600">₹{activity.amount}</b> payment from <b>{activity.performedFor[0]===userDetails._id?"You":userMap[activity.performedFor[0]]}</b></div>
                                    )
                                }
