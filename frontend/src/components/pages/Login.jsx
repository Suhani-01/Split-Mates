    ...
    <div className="bg-bg-secondary overflow-y-hidden h-full">
      ...
      <div className="min-h-[95vh] md:mx-auto overflow-y-hidden md:max-w-[1200px] grid md:grid-cols-[1fr_1fr] md:gap-3 items-center md:justify-center">
        ...
        <div className="bg-white mx-6 sm:mx-10 md:w-full md:mx-auto p-8 rounded-2xl shadow-md md:max-w-md">
          ...
          <div className="text-text-light mb-10 mt-3 text-center">Login to continue to your account</div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="relative">
              ...
              <input
                ...
                required
              />
              <div onClick={()=>setSeePassword((prev)=>!prev)} className="absolute text-text-light cursor-pointer right-2 text-2xl bottom-2">{seePassword ? <IoMdEye />:<IoMdEyeOff />}</div>
            </div>
            {errorMessage && <div className="text-center text-sm text-danger">{errorMessage}</div>}

            <button
            disabled={loading}
              type="submit"
              className="w-full cursor-pointer bg-primary text-white py-2 rounded-lg font-semibold hover:bg-primary-light transition"
            >
